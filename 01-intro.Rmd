...
```{r}
constraints %>%
  knitr::kable(caption="Constraints", escape = TRUE) %>%
  kableExtra::kable_styling()
```
...
cub_coffee_optima <- 
  ...
  mutate(
    ...
    level_label = paste0("К. кафе=", level)
  )
...
plt1_fs_poly_lines + 
  ...
  geom_abline(
      data =  tribble(
        ...
        "Кубинско кафе = 250", 250 / 0.75, -0.5 / 0.75,
        ),
  ...
  # annotate("text", x = 230, y = 60 - y_offset, label = "К. кафе=160", size = 3) +
  # annotate("text", x = 230, y = 100 - y_offset, label = "К. кафе=180", size = 3)+
  # annotate("text", x = 230, y = 140 - y_offset, label = "К. кафе=195", size = 3) +
  # annotate("text", x = 230, y = 220 - y_offset, label = "К. кафе=250", size = 3) +
  geom_point(
  ...
  geom_text(
    ...
    size = 3,
    position = position_nudge(10, 20)
    )
```
Пресечната точка на уравненията към "Търсене Делукс" и "Бразилско кафе" намираме
като решение на системата от две уравнения:
